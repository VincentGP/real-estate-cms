...
//When the DOM is loaded and you click on a element with the class link
$(document).on("click", ".link", function() {
	//Hide the wdw
	$(".wdw").hide();
	//Extract the attr from the window you clicked
	var sWindowToShow = $(this).attr("data-go-to");
	//Show the wdw with the attr you just clicked
	// $("#"+sWindowToShow).show();
	$("#"+sWindowToShow).css("display","flex");


	//Get the property id from the sibling and store it in a variable
	...
	//Select and create a property div for the id
...
$(document).on("click", ".btn-delete-property", function() {
	...
	$.getJSON(sUrl, function(jData) {
		if(jData.status == "ok") {
			//If the status is ok then remove the object you clicked on
			oTheParent.remove();
		}
	});
...
//Determines whether or not there is a super-admin in the database on page load
...
function fnHideMenu(){
	//Makes the menu slide away
	$("#menu").animate({"left":"-250px"},400);
	//Fades out the content cover
	$("#content-cover").fadeOut(400);
}

function fnShowMenu(){
	//Makes the element with the id menu animate in
	$("#menu").animate({"left":"0px"},400);
	//Fades in the content cover
	$("#content-cover").fadeIn(400);
}
...
function fnGetProperties(){
	...
	//Initiate Ajax and get data sent via the URL
	$.getJSON(sUrl, function(jData) {
...
function fnGetUsers(){
	...
	$.getJSON(sUrl, function(jData) {
		...
		var sUser = '<div class="lbl-user">\
							...
							<div data-go-to="wdw-create-user" class="fa fa-pencil-square-o fa-fw link"></div>\
...
function fnSaveProperty(){
	...
	//If there is no id, then create a new entry
	}else {
